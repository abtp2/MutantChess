 ...
 * Faithfully implementing reference/analysis.ts and reference/classification.ts
 *
 * A move is classified as BRILLIANT if:
 * 1. It is the engine's best/top move or tied with the best line.
 * 2. It intentionally sacrifices material (a candidate piece of value > captured piece is hanging, excluding King & pawns).
 * 3. The mover was not already in check prior to the move.
 * 4. The position is not already crushing (+700 cp or mate in 1-3) on the alternative line.
 * 5. It maintains an equal/winning position (>= 0) or is a forced mating attack for the mover.
 * 6. The sacrifice is genuine: at least one sacrificed piece is viably capturable by the opponent
 *    (i.e. not an illegal move or an attacker pinned to an equal/higher value piece).
 */
export function checkBrilliantMove({
  ...
  if (!isTopMove || !lastFen || !currentFen || !moveUci || moveUci.length < 4) {
    return false;
  }

  const moverColor = currentFen.includes(' b ') ? 'w' : 'b';
  const moverSign = moverColor === 'w' ? 1 : -1;

  const currentEvalVal = evaluation?.value ?? 0;
  const currentEvalType = evaluation?.type || 'cp';
  const moverAbsoluteEval = currentEvalVal * moverSign;

  const secondEvalVal = secondEvaluation?.value ?? 0;
  const secondEvalType = secondEvaluation?.type || 'cp';
  const moverSecondAbsoluteEval = secondEvalVal * moverSign;

  const isMatingAttack = currentEvalType === 'mate' && moverAbsoluteEval > 0;

  const winningAnyways = !isMatingAttack && (
    (secondEvaluation && moverSecondAbsoluteEval >= 700 && currentEvalType === 'cp') ||
    (secondEvaluation && secondEvalType === 'mate' && Math.abs(secondEvalVal) <= 3 && currentEvalType === 'mate')
  );

  if ((moverAbsoluteEval < 0 && !isMatingAttack) || winningAnyways) {
    return false;
  }
  ...
  try {
    ...
    const moveTo = moveUci.slice(2, 4);
    const lastPiece = lastBoard.get(moveTo) || { type: 'm' };
    const lastPieceVal = pieceValues[lastPiece.type] || 0;
    ...
    for (const row of currentBoard.board()) {
      for (const piece of row) {
        ...
        // If the piece captured by this move had higher or equal value than the candidate piece,
        // then this is a favorable or equal trade rather than a piece sacrifice.
        if (lastPieceVal >= (pieceValues[piece.type] || 0)) {
          continue;
        }

        // Check if the piece is hanging / sacrificed in the current position
        if (isPieceHanging(lastFen, currentFen, piece.square)) {
          ...
          try {
            ...
            for (const row of captureTestBoard.board()) {
              for (const enemyPiece of row) {
                ...
                if (
                  isPieceHanging(currentFen, captureTestBoard.fen(), enemyPiece.square) &&
            ...
            // Ignore illegal moves / exceptions
          } finally {
            if (moveMade) {
...
export async function analyzeFullGame(moves, onProgress, options = {}) {
  const depthParam = typeof options === 'number' ? options : (options.depth || 8);
  const movetimeParam = typeof options === 'object' && options.movetime ? options.movetime : 150;
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (analysisWorker) {
      evalResult = await new Promise((resolve) => {
        ...
        }, Math.max(500, movetimeParam + 250));
