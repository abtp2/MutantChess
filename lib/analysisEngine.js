...
// Built purely on local Stockfish 18 WASM with tactical evaluation and classification algorithms
...
import { DEFAULT_ANALYSIS_DEPTH, MIN_ANALYSIS_DEPTH } from './settings.js';

export { DEFAULT_ANALYSIS_DEPTH, MIN_ANALYSIS_DEPTH };
...
import { considerBrilliantClassification } from './wintrchess/index.js';

export { considerBrilliantClassification };

/**
 * Rigorous Brilliant Move Classifier
 * Faithfully implementing WintrCat/wintrchess (shared/src/lib/reporter/classification/brilliant.ts)
 * and its supporting modules (pieceSafety, dangerLevels, pieceTrapped, criticalMove, attackers, defenders).
 *
 * A move is classified as BRILLIANT if:
 * 1. It is the engine's best/top move or tied with the best line.
 * 2. It is a critical move candidate (not crushing on line 2, not in losing position, not promotion, not escaping check).
 * 3. It intentionally leaves or creates unsafe pieces of value > captured piece.
 * 4. It does not move a piece to safety (unless in check).
 * 5. The sacrifice is not protected by danger levels (counterthreats of >= value or mate pins).
 * 6. The sacrificed piece was not already trapped.
 */
export function checkBrilliantMove({
  lastFen,
  currentFen,
  moveUci,
  evaluation,
  ...
  if (!isTopMove || !lastFen || !currentFen || !moveUci) {
    return false;
  }
  ...
  try {
    ...
    try {
      if (moveUci.length >= 4 && /^[a-h][1-8][a-h][1-8]/i.test(moveUci)) {
        playedMove = lastBoard.move({
          from: moveUci.substring(0, 2).toLowerCase(),
          to: moveUci.substring(2, 4).toLowerCase(),
          promotion: moveUci.length > 4 ? moveUci[4].toLowerCase() : undefined,
        });
      } else {
        playedMove = lastBoard.move(moveUci);
      }
    } catch (e) {
      try {
        playedMove = lastBoard.move(moveUci);
    ...
    const moverColor = playedMove.color; // 'w' or 'b'
    const moverMultiplier = moverColor === 'w' ? 1 : -1;

    const subjectiveEvaluation = {
      type: evaluation?.type === 'mate' ? 'mate' : 'centipawn',
      value: (evaluation?.value ?? 0) * moverMultiplier,
    };

    const previousSubjectiveEvaluation = previousEvaluation
      ? {
          type: previousEvaluation.type === 'mate' ? 'mate' : 'centipawn',
          value: (previousEvaluation.value ?? 0) * moverMultiplier,
        }
      : undefined;

    const secondSubjectiveEvaluation = secondEvaluation
      ? {
          type: secondEvaluation.type === 'mate' ? 'mate' : 'centipawn',
          value: (secondEvaluation.value ?? 0) * moverMultiplier,
        }
      : undefined;

    const prevNode = {
      board: new Chess(lastFen),
      evaluation: previousEvaluation || { type: 'centipawn', value: 0 },
      subjectiveEvaluation: previousSubjectiveEvaluation,
      secondSubjectiveEvaluation: secondSubjectiveEvaluation,
      topMove: playedMove,
    };
    ...
    return considerBrilliantClassification(prevNode, currNode);
  } catch (err) {
    return false;
  }
}

// Full Position Classifier matching official tactical classification standards
export function analysePositions(positions) {
  ...
  for (const position of positions.slice(1)) {
    ...
    // 2. If it is the top line, disregard other detections and give best (matching analysis.ts line 93)
    ...
    if (isTopEngineMove || isTiedWithTop) {
    ...
    // If current verdict is best, check for possible brilliancy (matching analysis.ts lines 159-251)
    if (position.classification === Classification.BEST) {
      const isBrilliant = checkBrilliantMove({
        ...
        moveUci: position.move?.uci,
        evaluation,
        previousEvaluation: topMove.evaluation,
        secondEvaluation: secondTopMove?.evaluation,
        isTopMove: true,
      });
      ...
      // Test for great move classification (matching analysis.ts lines 253-263)
    ...
    // Do not allow blunder if move still completely winning (matching analysis.ts lines 265-268)
    if (position.classification === Classification.BLUNDER && absoluteEvaluation >= 600) {
      position.classification = Classification.GOOD;
    }

    // Do not allow blunder if you were already in a completely lost position (matching analysis.ts lines 270-273)
  ...
  // Generate opening names for each position using openings database
...
export async function analyzeFullGame(moves, onProgress, options = {}) {
  const rawDepth = typeof options === 'number' ? options : (options.depth || DEFAULT_ANALYSIS_DEPTH);
  const depthParam = Math.max(MIN_ANALYSIS_DEPTH, rawDepth);
  const movetimeParam = typeof options === 'object' && options.movetime ? options.movetime : Math.max(400, depthParam * 35);
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (analysisWorker) {
      evalResult = await new Promise((resolve) => {
        ...
        }, Math.max(800, movetimeParam + 400));
        ...
        const handleMsg = (e) => {
          ...
          if (line.startsWith('info') && line.includes('score')) {
            ...
            if (!resolved) {
              ...
              // If we have not received any evaluation info for this search yet, this bestmove
              // is a residual output emitted while stopping a prior search. Safely ignore it!
              if (Object.keys(topMovesMap).length === 0) {
                return;
              }
        ...
        analysisWorker.postMessage(`go depth ${depthParam} movetime ${movetimeParam}`);
      });
    } else {
  ...
  const evaluations = positions.map((p) => {
    ...
    const cpVal = (p.whiteCp !== undefined && p.whiteCp !== null)
      ? p.whiteCp
      : (top?.evaluation?.type === 'cp' ? top.evaluation.value : (top?.evaluation?.value > 0 ? 10000 : (top?.evaluation?.value < 0 ? -10000 : 0)));
    const mateVal = (p.whiteMate !== undefined && p.whiteMate !== null)
      ? p.whiteMate
      : (top?.evaluation?.type === 'mate' ? top.evaluation.value : null);
