...
class StockfishService {
  ...
  async evaluatePosition({ fen, depth = 12, multiPv = 3 }) {
    await this.init();
    ...
    return new Promise((resolve) => {
      ...
      const finalizeEval = () => {
        ...
        const topMoves = Object.values(topMovesMap)
        ...
        return {
          ...
          mate: whiteMate,
          whiteCp,
          topMoves: topMoves.length > 0 ? topMoves : (lastInfo.bestMove ? [{ multipv: 1, uci: lastInfo.bestMove, cp: whiteCp, mate: whiteMate, pv: lastInfo.pv, depth: lastInfo.depth }] : []),
        };
      };
      ...
      try {
        ...
        this.evalWorker.postMessage(`setoption name MultiPV value ${multiPv || 3}`);
