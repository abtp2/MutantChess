...
class StockfishService {
  ...
  async evaluatePosition({ fen, depth = DEFAULT_ANALYSIS_DEPTH, multiPv = 2, onUpdate = null }) {
    await this.init();
    const targetDepth = Math.max(MIN_ANALYSIS_DEPTH, depth || DEFAULT_ANALYSIS_DEPTH);
    ...
    return new Promise((resolve) => {
      ...
      const timeoutMs = Math.max(2500, targetDepth * 200);
      const timer = setTimeout(() => {
      ...
      try {
        ...
        searchStarted = true;
        this.evalWorker.postMessage(`go depth ${targetDepth}`);
      } catch (e) {
        if (!resolved) {
          resolved = true;
          clearTimeout(timer);
